...
app.get('/api/documents/:id/download', async (req, res) => {
    ...
    try {
        ...
        if (!document) {
            return res.status(400).json({ error: 'Document non trouvé' });
        }
...
app.delete('/api/courses/:id', async (req, res) => {
    ...
    try {
        ...
        const filePath = path.join(UPLOADS_DIR, course.name);
...
app.delete('/api/grades/:id', async (req, res) => {
    ...
    try {
        ...
        if (grade.subject) {
            const filePath = path.join(UPLOADS_DIR, grade.subject);
            await fs.unlink(filePath).catch(err => {
                console.error('Erreur lors de la suppression du fichier:', err.message);
            });
        }
